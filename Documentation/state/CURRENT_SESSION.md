# Current Session — Lowborn

**Proiect:** Lowborn
**Dată:** 2026-07-22
**Fază:** Game Design — Arena Slots final, gata de G1

## Obiectiv curent
Lowborn — auto-battler single-player cu card crafting din părți.
**Offline-first** — 100% local, zero server, AI opponent adaptiv.
Tech stack: PixiJS (TypeScript pur) + Tauri.

## Status
- [x] Repo inițializat
- [x] LOWBORN_GDD.md — document complet de design
- [x] README.md actualizat (offline-first)
- [x] State files actualizate
- [x] Tech stack decis: PixiJS pur / TypeScript / Tauri (D009)
- [x] Direcție offline-first decisă (D008)
- [x] Cleanup complet: Godot șters, Grid → Arena Slots, drop rates offline, economie P1
- [ ] G1 — Arena Slots design aprobat
- [ ] P1 — Prototype offline (Arena Slots + snap + auto-battle + AI)
